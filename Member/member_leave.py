...
class MemberLeave(ctk.CTkFrame):
    ...
    def show_form_view(self):
        ...
        ctk.CTkOptionMenu(inner, values=["Sick Leave", "Casual Leave", "Vacation"], variable=self.type_var, width=250).grid(row=4, column=0, columnspan=2, padx=10, pady=(0, 15), sticky="w")

        # --- ROW 5: ADD HALF DAY (+0.5) ---
    ...
    def submit_leave(self):
        ...
        if total <= 0:
            messagebox.showerror("Error", "Duration cannot be 0. Please enable range or half-day.")
            return
        ...
        try:
            sql = """INSERT INTO leave_requests (user_id, leave_type, start_date, end_date, reason, is_half_day) 
                     VALUES (%s, %s, %s, %s, %s, %s)"""
            self.db.cursor.execute(sql, (self.user['id'], l_type, start, end, reason, is_hd))
            self.db.conn.commit()
    ...
    def refresh_list(self):
        ...
        # Logic to display correct count in history
        query = "SELECT *, (DATEDIFF(end_date, start_date) + 1) as diff FROM leave_requests WHERE user_id = %s ORDER BY id DESC"
        ...
        for r in self.db.cursor.fetchall():
            ...
            # If start == end and it wasn't a range, diff is 1. If is_hd is 1, total is diff - 1 + 0.5? 
            # Simplified: Let's assume if end_date was set same as start, diff is 1.
            # If user didn't use range, we only show the 0.5.
            if r['start_date'] == r['end_date'] and r['is_half_day']:
                days = 0.5
            else:
                days = float(r['diff']) + (0.5 if r['is_half_day'] else 0)
            ...
            ctk.CTkLabel(card, text=r['status'], text_color=status_color.get(r['status'], "gray"), font=("Arial", 12, "bold")).pack(side="right", padx=15)