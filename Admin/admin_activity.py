...
class AdminAnnouncements(ctk.CTkFrame):
    def __init__(self, master, user_data):
        ...
        self.toggle_btn = ctk.CTkButton(self.header_frame, text="+ Create New", width=120, 
                                        fg_color="#6366F1", command=self.toggle_form)
        ...
        self.post_btn = ctk.CTkButton(self.btn_row, text="Post", width=100, 
                                      fg_color="#6366F1", command=self.handle_save)
        self.post_btn.pack(side="right")

        # --- 3. LIST AREA ---
        self.list_frame = ctk.CTkScrollableFrame(self, label_text="Admin Post History")
    ...
    def toggle_form(self):
        if not self.form_visible:
            self.form_frame.pack(after=self.header_frame, fill="x", padx=20, pady=10)
    ...
    def reset_form(self):
        ...
        self.msg_ent.insert("0.0", self.placeholder)
        self.msg_ent.configure(text_color="gray")
        self.post_btn.configure(text="Post", fg_color="#6366F1")
        self.toggle_btn.configure(text="+ Create New", fg_color="#6366F1")
        self.form_visible = False
    ...
    def refresh_list(self):
        ...
        try:
            ...
            for row in self.db.cursor.fetchall():
                ...
                ctk.CTkLabel(header_f, text=post_time, font=("Arial", 10), text_color="gray60").pack(side="left", padx=15)
                
                ctk.CTkLabel(text_container, text=row['message'], font=("Arial", 12), wraplength=450, justify="left", anchor="w", text_color="gray80").pack(fill="x", pady=(2, 0))
