...
class AdminAttendance(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # 🔥 IMPORTANT: make grid responsive
        for i in range(14):
        ...
        self.from_date_btn = DatePickerButton(
            control_card,
            initial_date=self.default_from_date,
            height=36,
            corner_radius=8
        )
        ...
        self.to_date_btn = DatePickerButton(
            control_card,
            initial_date=self.default_to_date,
            height=36,
            corner_radius=8
        )
        ...
        action_frame.grid(row=0, column=8, columnspan=6, sticky="ew")
        ...
        self.result_label = ctk.CTkLabel(
            self,
            text="0 records | Month Work: - | Monthly Hours: -",
    ...
    # main container
        ...
        h("Team", 120).grid(row=0, column=2, padx=0, pady=8, sticky="w")
        h("Work Days", 100).grid(row=0, column=3, padx=0, pady=8, sticky="w")
        h("Working Hours", 120).grid(row=0, column=4, padx=0, pady=8, sticky="w")
        h("Leave Days", 100).grid(row=0, column=5, padx=0, pady=8, sticky="w")
        h("Late Count", 100).grid(row=0, column=6, padx=0, pady=8, sticky="w")
        h("OT Hours", 100).grid(row=0, column=7, padx=(0, 10), pady=8, sticky="w")
        header.grid_columnconfigure(8, weight=1)
        # ?E?・ｽ・ｽ?E? SCROLL FRAME (like AdminUsers)
    ...
    def _open_checkin_editor(self, row, parent_card):
        ...
        # safer focus
        self.after_idle(
            lambda e=entry: e.focus_set()
            if e.winfo_exists() else None
        )
        ...
        # safer save wrapper
        def handle_save(event=None, r=row, w=entry):
            try:
                if w.winfo_exists():
                    self._save_check_in_edit(r, w)
            except Exception:
                pass
    ...
    def _save_check_in_edit(self, row, entry_widget):
        ...
        normalized = (
            self._normalize_checkin_time(new_value)
        ...
        if (
            (current_value == "-" and not new_value)
            or
            (normalized and current_value == normalized)
        ):
            self.after_idle(self._refresh_employee_detail)
            return

        user_id = (
            self._detail_export_context.get("user_id")
            if self._detail_export_context
            else None
        )
        ...
        if not user_id:
            messagebox.showerror(
                "Save Error",
                "Unable to identify the current employee record.",
            )
            return
    ...
    @staticmethod
    def _safe_filename(value, fallback="Attendance"):
    ...
    def _actual_hours_from_row(self, row):
        ...
        if leave_type == "morning_half":
            ...
            # No leave: actual hours are based on real checkout only.
            work_end = check_out
    ...
    def _get_employee_attendance_rows(self, user_id, month_key="", from_date=None, to_date=None):
        sql = """
        SELECT
    ...
    def _get_payroll_range(self, from_date, to_date):
        ...
        """
        # If user already selects full range, we still respect it
        start = from_date
        end = to_date
    ...
    def _open_employee_attendance_detail(self, user_id, employee_id, full_name, team_name=None, from_date=None, to_date=None):
        ...
        try:
            team_value = self.team_filter.get()
        except Exception:
            team_value = self._saved_filters.get("team", "All Teams") if hasattr(self, "_saved_filters") else "All Teams"
    ...
    def _export_employee_detail_pdf(self):
        ...
        try:
            ...
            safe_month_hours = escape(f"{self._month_hours_from_workdays(month_workdays)}")
            ...
            for row in rows:
                ...
                table_data.append([
                    ...
                    self._fmt_hours(self._ot_hours_from_row(row)),
    ...
    def _export_employee_detail_excel(self):
        ...
        safe_name = self._safe_filename(full_name, fallback="Employee")
        safe_month = self._safe_filename(month_label, fallback="Month")
        ...
        try:
            ...
            # try:
            #     if (
            #         hasattr(self, "team_filter")
            #         and self.team_filter.winfo_exists()
            #     ):
            #         team_name = self.team_filter.get().strip() or "-"
    ...
    def _calculate_workdays_range(self, from_date, to_date):
            return sum(
                1
    ...
    # clear old rows (scroll frame)
        ...
        self.month_total_workdays = str(self._calculate_workdays_range(from_date, to_date))
    ...
    # ================= SEARCH FILTER =================
        outer_where = "WHERE (a.latest_date IS NOT NULL OR o.overtimehour IS NOT NULL) AND (u.full_name LIKE %s OR CAST(u.employee_id AS CHAR) LIKE %s)"
            ...
            # ================= CREATE CARDS =================
                for row in self.current_rows:
                    ...
                    row_working_hours = sum(
                        self._actual_hours_from_row(r) for r in detail_rows
                    )
                    ...
                    # row content
                    ctk.CTkLabel(card, text=self._dash(row["employee_id"]), width=80, anchor="w").grid(row=0, column=0, padx=(10, 0), pady=8, sticky="w")
                    ctk.CTkButton(
                        card,
                        ...
                        font=("Arial", 12),
                        cursor="hand2",
                        command=lambda user_id=row["user_id"], employee_id=row["employee_id"], full_name=row["full_name"], team_name=row.get("team_name"): self._open_employee_attendance_detail(user_id, employee_id, full_name, team_name),
                    ...
                    border.grid(row=1, column=0, columnspan=9, sticky="ew", pady=(6, 0))
    ...
    def export_pdf(self):
        ...
        try:
            ...
            for row in self.current_rows:
                detail_rows = self._get_employee_attendance_rows(row["user_id"], "", from_date, to_date)
                row_workdays = sum(int(r.get("workdays", 0) or 0) for r in detail_rows)
    ...
    def export_excel_file(self):
        ...
        selected_team = self._safe_filename(self.team_filter.get(), fallback="All_Teams")

        file_name = f"Attendance_{selected_team}_{from_str}_to_{to_str}.xlsx"
    ...
    def _export_excel(self, file_path):
        ...
        try:
            ...
            ws[f"E{info_row}"] = self.team_filter.get()


            # Ensure from_date / to_date are available for detail queries
            try:
                from_date = self.from_date_btn.get_date()
                to_date = self.to_date_btn.get_date()
            except Exception:
