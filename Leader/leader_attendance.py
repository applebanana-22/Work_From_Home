...
class LeaderAttendance(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            header_f,
            text="Team Attendance Dashboard",
            font=("Arial", 24, "bold"),
    ...
    ).grid(row=0, column=8, padx=(348, 20), pady=8) # Changed 6 to 100 for more margin-left
    ...
    # main container
        ...
        # 櫨 SCROLL FRAME (like AdminUsers)
    ...
    def reset_filters(self):
        ...
        # 2. Calculate current payroll month
        today = datetime.now()
        if today.day >= 26:
            if today.month == 12:
                current_payroll_ym = f"{today.year + 1}-01"
    ...
    # ================= SEARCH FILTER =================
        outer_where = "WHERE u.team_id = %s AND (u.full_name LIKE %s OR CAST(u.id AS CHAR) LIKE %s)"
        outer_params = [
            self.user["team_id"],   # 櫨 leader's team
            f"%{search_query}%",
            f"%{search_query}%"
        ]
        ...
        sql = f"""
            ...
            AND (u.full_name LIKE %s OR CAST(u.id AS CHAR) LIKE %s)
            ...
            # ================= CREATE CARDS =================
                for row in self.current_rows:
                    ...
                    ctk.CTkButton(
                            ...
                            font=("Arial", 12),
                            cursor="hand2",
                            command=lambda user_id=row["id"], full_name=row["full_name"], team_name=row.get("team_name"):
                                self._open_employee_attendance_detail(user_id, full_name, team_name),
                    ...
                    # 櫨 bottom border line (like AdminUsers style)
    ...
    def export_pdf(self):
        ...
        try:
            ...
            table = Table(
                table_data,
                repeatRows=1,
                colWidths=[0.9 * inch, 2.4 * inch, 1.15 * inch, 1.05 * inch, 1.0 * inch, 1.0 * inch],
            )
            table.setStyle(TableStyle([
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#0F172A")),
                ("TEXTCOLOR", (0, 0), (-1, 0), colors.white),
