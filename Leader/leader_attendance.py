...
class LeaderAttendance(ctk.CTkFrame):
    ...
    # ================= SEARCH FILTER =================
        ...
        outer_params = [
            self.user["team_id"],   # 🔥 leader's team
            f"%{search_query}%",
            f"%{search_query}%"
        ]
            ...
            # ================= CREATE CARDS =================
                for row in self.current_rows:
                    ...
                    ctk.CTkButton(
                            ...
                            cursor="hand2",
                            command=lambda user_id=row["id"], full_name=row["full_name"]:
                                self._open_employee_attendance_detail(user_id, full_name),
